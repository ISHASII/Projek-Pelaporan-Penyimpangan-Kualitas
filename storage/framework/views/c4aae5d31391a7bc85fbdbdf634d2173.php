<?php $__env->startSection('content'); ?>
    ...
    <div id="approve-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40">
        <div class="bg-white rounded-lg shadow-lg w-full max-w-lg p-6">
            ...
            <div class="mb-4">
                <p class="text-sm text-gray-600 mb-2">Pilih Sect Head yang ingin menerima email approve:</p>
                ...
                <div class="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto border rounded p-2 bg-white">
                    <?php $__empty_1 = true; $__currentLoopData = $sectApprovers ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sa): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <label class="inline-flex items-center gap-2 text-sm text-gray-700">
                            <input type="checkbox" name="approve_recipients[]" value="<?php echo e($sa->npk); ?>" class="approve-recipient-checkbox">
                            <span class="truncate"><?php echo e($sa->name); ?> <?php if($sa->email): ?> &lt;<?php echo e($sa->email); ?>&gt; <?php endif; ?></span>
                        </label>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                        <div class="text-sm text-gray-500 italic">Tidak ada approver Sect Head QA yang tersedia.</div>
                    <?php endif; ?>
                </div>
                <div class="text-xs text-gray-500 mt-1">Tidak memilih siapa pun akan mengirim ke semua Sect Head QA.</div>
            </div>
    ...
    <div id="request-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40">
        <div class="bg-white rounded-lg shadow-lg w-full max-w-lg p-6">
            ...
            <p id="request-modal-msg" class="text-sm text-gray-700 mb-4">Apakah Anda yakin ingin mengirim request approval untuk NQR ini?</p>

            <div class="mb-4">
                <p class="text-sm text-gray-600 mb-2">Pilih Foreman yang ingin menerima email request:</p>
                <div class="mb-2 flex items-center justify-between">
                    <div class="text-xs text-gray-500">Pilih penerima:</div>
                    <div class="text-xs text-gray-500"><label class="inline-flex items-center gap-2"><input type="checkbox" id="request-recipients-select-all"> Pilih semua</label></div>
                </div>
                <div class="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto border rounded p-2 bg-white">
                    <?php $__empty_1 = true; $__currentLoopData = $foremanApprovers ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fa): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <label class="inline-flex items-center gap-2 text-sm text-gray-700">
                            <input type="checkbox" name="recipients[]" value="<?php echo e($fa->npk); ?>" class="recipient-checkbox">
                            <span class="truncate"><?php echo e($fa->name); ?> <?php if($fa->email): ?> &lt;<?php echo e($fa->email); ?>&gt; <?php endif; ?></span>
                        </label>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                        <div class="text-sm text-gray-500 italic">Tidak ada approver Foreman QA yang tersedia.</div>
                    <?php endif; ?>
                </div>
                <div class="text-xs text-gray-500 mt-1">Tidak memilih siapa pun akan mengirim ke semua Foreman QA.</div>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            approveForm.addEventListener('submit', function (e) {
                ...
                // Get selected recipients for approve
                const checkedRecipients = Array.from(approveModal.querySelectorAll('input[name="approve_recipients[]"]:checked')).map(i => i.value);
            ...
            requestForm.addEventListener('submit', function (e) {
                ...
                // Get selected recipients for request
            ...
            // Select all recipients checkboxes
            const approveSelectAll = document.getElementById('approve-recipients-select-all');
            if (approveSelectAll) {
                approveSelectAll.addEventListener('change', function () {
                    approveModal.querySelectorAll('input[name="approve_recipients[]"]').forEach(cb => cb.checked = this.checked);
                });
            }
            const requestSelectAll = document.getElementById('request-recipients-select-all');
            if (requestSelectAll) {
                requestSelectAll.addEventListener('change', function () {
                    requestModal.querySelectorAll('input[name="recipients[]"]').forEach(cb => cb.checked = this.checked);
                });
            }
