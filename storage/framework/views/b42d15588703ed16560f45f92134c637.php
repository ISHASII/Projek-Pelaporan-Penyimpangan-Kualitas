		...
		<div class="m-0">
			<div
				...
				<div class="p-6">
					...
					<form id="filter-form" method="GET" action="<?php echo e(route('ppchead.cmr.index')); ?>" class="mb-4">
						...
						<div class="rounded-md border border-gray-200 p-3 sm:p-4 bg-white shadow-sm">
							...
							<div class="hidden lg:flex gap-2 items-end">
								...
								<div class="flex gap-2 items-center flex-shrink-0">
									<button type="submit"
										class="inline-flex items-center px-3 py-1.5 bg-yellow-600 hover:bg-yellow-700 text-white text-sm font-medium rounded-md">Apply</button>
					...
					<div class="responsive-table overflow-x-auto rounded-md ring-1 ring-gray-50">
												...
												<div class="flex flex-col items-center justify-center gap-2">
													<div class="flex items-center justify-center gap-4 action-buttons-container">
														<?php
															...
															$agm = strtolower($cmr->agm_status ?? 'pending');
															$ppc = strtolower($cmr->ppchead_status ?? 'pending');
															$statusApproval = strtolower($cmr->status_approval ?? '');
															$ppcComplete = false;
															try {
																$note = $cmr->ppchead_note ?? null;
																if (!empty($note)) {
																	$decoded = json_decode($note, true);
																	if (is_array($decoded)) {
																		if (isset($decoded['ppc']) || isset($decoded['disposition']) || isset($decoded['nominal']) || isset($decoded['shipping'])) {
																			$ppcComplete = true;
																		} else {
																			foreach ($decoded as $k => $v) {
																				if (is_array($v) && (isset($v['disposition']) || isset($v['nominal']))) {
																					$ppcComplete = true;
																					break;
																				}
																			}
														...
														<div class="flex flex-col items-center justify-center gap-2">
															...
															<div class="flex flex-row items-center justify-center gap-6">
																<?php if(!is_null($cmr->requested_at_qc) && $sect === 'approved' && $dept === 'approved' && $agm === 'approved' && $ppc === 'pending' && $statusApproval === 'waiting for ppc head approval'): ?>
																	<div class="flex flex-col items-center gap-1">
																		<a href="<?php echo e(route('ppchead.cmr.ppcForm', $cmr->id)); ?>"
                                                                ...
                                                                <?php if(!is_null($cmr->requested_at_qc) && $sect === 'approved' && $dept === 'approved' && $agm === 'approved' && $ppc === 'pending' && $statusApproval === 'waiting for ppc head approval'): ?>
																	<div class="flex flex-col items-center gap-1">
																		<button type="button"
																			...
																			title="Tolak"
																			data-url="<?php echo e(route('ppchead.cmr.reject', $cmr->id)); ?>"
																...
																<?php if(!is_null($cmr->requested_at_qc)): ?>
																	<div class="flex flex-col items-center gap-1">
																		<a href="<?php echo e(route('ppchead.cmr.previewFpdf', $cmr->id)); ?>"
																			target="_blank"
						...
						<?php else: ?>
							<div class="text-center py-12 text-gray-500">
								<div class="text-lg font-medium">No CMR data</div>
								<div class="text-sm">No CMR matches the selected filters.</div>
							</div>
						<?php endif; ?>
					</div>
...
<!-- Approve and Reject modals for PPC Head (CMR) -->
	...
	<div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
		...
		<div class="mb-4">
			...
			<div class="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto border rounded p-2 bg-white">
				<?php $__empty_1 = true; $__currentLoopData = $vddApprovers ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $approver): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
					<label class="inline-flex items-center gap-2 text-sm text-gray-700">
						<input type="checkbox" name="recipients[]" value="<?php echo e($approver->npk); ?>" class="approve-recipient-checkbox">
						<span class="truncate"><?php echo e($approver->name); ?> <?php if($approver->email): ?> &lt;<?php echo e($approver->email); ?>&gt; <?php endif; ?></span>
					</label>
...
<div id="reject-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40">
	<div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
		<h3 class="text-lg font-semibold mb-4">Confirm Reject</h3>
		<p id="reject-modal-msg" class="text-sm text-gray-700 mb-6">Are you sure you want to reject this CMR?</p>
		<div class="flex justify-end gap-3">
			...
			<form id="reject-form" method="POST" action="">
				<?php echo csrf_field(); ?>
				<button type="submit"
					class="inline-flex items-center h-10 px-4 text-sm font-medium rounded-md bg-red-600 text-white hover:bg-red-700">Reject</button>
			</form>
		</div>
	</div>
</div>
...
<div id="ppc-redirect-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40">
	<div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
		<h3 class="text-lg font-semibold mb-2">PPC Form incomplete</h3>
		<p id="ppc-redirect-msg" class="text-sm text-gray-700 mb-4">You must fill the PPC form before approving.</p>
		<div class="flex justify-end gap-3">
			<button id="ppc-redirect-cancel" type="button"
				class="inline-flex items-center h-10 px-4 text-sm font-medium rounded-md bg-gray-100 hover:bg-gray-200">Cancel</button>
			<button id="ppc-redirect-open" type="button"
				class="inline-flex items-center h-10 px-4 text-sm font-medium rounded-md bg-yellow-500 text-white hover:bg-yellow-600">Open
				Form</button>
		</div>
	</div>
</div>
	...
	document.addEventListener('DOMContentLoaded', function () {
			...
			if (data.hide_actions) {
				...
				if (actionsContainer) {
					...
					if (pdfLink) {
						...
						var innerContainer = actionsContainer.querySelector('.flex.flex-row');
						if (innerContainer && pdfParent) innerContainer.appendChild(pdfParent);
					} else {
		...
		document.querySelectorAll('.open-approve-modal').forEach(btn => {
			btn.addEventListener('click', function () {
				...
				// If PPC inputs are not complete, show redirect modal
				if (!ppcComplete && ppcUrl) {
					...
					ppcRedirectMsg.textContent = 'PPC form has not been filled. You must fill the PPC form before approving. Open the form now?';
		...
		document.querySelectorAll('.open-reject-modal').forEach(btn => {
			btn.addEventListener('click', function () {
				...
				rejectMsg.textContent = 'Are you sure you want to reject CMR ' + (noreg || '') + '?';
...
<?php echo $__env->make('layouts.navbar', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ilham\Documents\PROJEK-LPK\resources\views/ppchead/cmr/index.blade.php ENDPATH**/ ?>