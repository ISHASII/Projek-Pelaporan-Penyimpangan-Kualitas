        ...
        <div class="m-0">
            <div
                ...
                <div class="p-6">
                    ...
                    <form id="filter-form" method="GET" action="{{ route('secthead.cmr.index') }}" class="mb-4">
                        @php
                            $dateValue = '';
                            if (request('date')) {
                                try {
                        ...
                        <div class="rounded-md border border-gray-200 p-3 sm:p-4 bg-white shadow-sm">
                            {{-- keep hidden date field for existing controller compatibility --}}
                            ...
                            <div class="block lg:hidden space-y-2">
                                ...
                                <div class="grid grid-cols-3 gap-2 pt-1">
                                    ...
                                    <a href="{{ route('secthead.cmr.index') }}"
                                            ...
                                            <td class="px-3 py-3 text-center text-sm hidden sm:table-cell action-buttons-container">
                                                <div class="flex flex-col items-center justify-center gap-2">
                                                    <div class="flex items-center justify-center gap-4">
                                                        @php $sectStatus = strtolower($cmr->secthead_status ?? 'pending'); @endphp
                                                        @if(!is_null($cmr->requested_at_qc) && $sectStatus === 'pending')
                                                            <div class="flex flex-col items-center gap-1">
                                                                <button type="button"
                                                                    class="open-approve-modal inline-flex items-center justify-center w-8 h-8 rounded-md hover:bg-green-50"
                                                                    title="Setuju"
                                                                    data-url="{{ route('secthead.cmr.approve', $cmr->id) }}"
                                                            ...
                                                            <div class="flex flex-col items-center gap-1">
                                                                <button type="button"
                                                                    ...
                                                                    title="Tolak"
                                                                    data-url="{{ route('secthead.cmr.reject', $cmr->id) }}"
                                                                    data-cmr-id="{{ $cmr->id }}" data-noreg="{{ $cmr->no_reg }}">
                                                        ...
                                                        @if(!is_null($cmr->requested_at_qc))
                                                            <div class="flex flex-col items-center gap-1">
                                                                <a href="{{ route('secthead.cmr.previewFpdf', $cmr->id) }}"
                                                                    target="_blank"
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function updateRowAfterAction(cmrId, newStatus, hideActions) {
                    ...
                    if (hideActions) {
                        ...
                        if (actionCell) {
                            actionCell.innerHTML = '<div class="flex flex-col items-center gap-1"><a href="{{ url("secthead/cmr") }}/' + cmrId + '/preview-fpdf" target="_blank" class="inline-flex items-center justify-center w-8 h-8 rounded-md hover:bg-gray-100" title="Preview PDF"><img src="{{ asset("icon/pdf.ico") }}" alt="PDF" class="w-4 h-4" /></a><span class="text-xs text-gray-500 mt-1">PDF</span></div>';
                        }
                    }
                }
