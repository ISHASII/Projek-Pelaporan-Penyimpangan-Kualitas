    ...
    <div class="w-full m-0 p-0">
        <div class="bg-white rounded-b-lg shadow-sm overflow-hidden w-full">
            ...
            <div class="max-w-screen-xl mx-auto px-6 py-6">
                <form action="{{ route('foreman.nqr.update', ['nqr' => $nqr->id]) }}" method="POST"
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" value="{{ $nqr->no_reg_nqr }}" readonly
                    ...
                    <div class="form-compact">
                        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 mb-6">
                            ...
                            <div>
                                ...
                                <input type="hidden" name="tgl_terbit_nqr" id="tgl_terbit_nqr"
                                    value="{{ old('tgl_terbit_nqr', $nqr->tgl_terbit_nqr->format('Y-m-d')) }}" required>
                                <input type="text" id="tgl_terbit_nqr_display" inputmode="numeric"
                                    ...
                                    value="{{ old('tgl_terbit_nqr') ? date('d-m-Y', strtotime(old('tgl_terbit_nqr'))) : $nqr->tgl_terbit_nqr->format('d-m-Y') }}">
                            ...
                            <div>
                                ...
                                <input type="hidden" name="tgl_delivery" id="tgl_delivery"
                                    value="{{ old('tgl_delivery', $nqr->tgl_delivery->format('Y-m-d')) }}" required>
                                <input type="text" id="tgl_delivery_display" inputmode="numeric" pattern="\d{2}-\d{2}-\d{4}"
                                    placeholder="dd-mm-yyyy"
                                    class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
                                    value="{{ old('tgl_delivery') ? date('d-m-Y', strtotime(old('tgl_delivery'))) : $nqr->tgl_delivery->format('d-m-Y') }}">
                        ...
                        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 mb-6">
                            ...
                            <div id="supplier_dropdown" class="relative">
                                ...
                                <input type="hidden" name="nama_supplier" id="nama_supplier"
                                    value="{{ old('nama_supplier', $nqr->nama_supplier) }}" required>
                                <button type="button" id="nama_supplier_btn"
                                    ...
                                    <span
                                        id="nama_supplier_btn_text">{{ old('nama_supplier', $nqr->nama_supplier) ?: 'Pilih atau ketik nama supplier' }}</span>
                            ...
                            <div id="part_dropdown" class="relative">
                                ...
                                <input type="hidden" name="nomor_part" id="nomor_part"
                                    value="{{ old('nomor_part', $nqr->nomor_part) }}" required>
                                <input type="hidden" name="nama_part" id="nama_part"
                                    value="{{ old('nama_part', $nqr->nama_part) }}">
                                <button type="button" id="nomor_part_btn"
                                    class="w-full text-left border rounded-lg px-3 py-2 text-sm flex justify-between items-center focus:outline-none focus:ring-2 focus:ring-red-500">
                                    <span
                                        id="nomor_part_btn_text">{{ (old('nomor_part', $nqr->nomor_part) ? old('nomor_part', $nqr->nomor_part) . ' — ' . old('nama_part', $nqr->nama_part) : 'Pilih atau cari nomor part') }}</span>
                                    <img src="{{ asset('icon/dropdown.ico') }}" alt="dropdown" class="w-4 h-4">
                                </button>
                        ...
                        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 mb-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="nomor_po" value="{{ old('nomor_po', $nqr->nomor_po) }}" required
                                    class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
                        ...
                        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 mb-6">
                            ...
                            <div>
                                ...
                                <select name="status_nqr" required
                                    ...
                                    <option value="Claim" {{ old('status_nqr', $nqr->status_nqr) == 'Claim' ? 'selected' : '' }}>Claim</option>
                                    <option value="Complaint (Informasi)" {{ old('status_nqr', $nqr->status_nqr) == 'Complaint (Informasi)' ? 'selected' : '' }}>Complaint (Informasi)</option>
                                </select>
                            ...
                            <div>
                                ...
                                <select name="location_claim_occur" required
                                    ...
                                    <option value="Receiving Insp" {{ old('location_claim_occur', $nqr->location_claim_occur) == 'Receiving Insp' ? 'selected' : '' }}>Receiving Insp
                                    </option>
                                    <option value="In-Process" {{ old('location_claim_occur', $nqr->location_claim_occur) == 'In-Process' ? 'selected' : '' }}>In-Process</option>
                                    <option value="Customer" {{ old('location_claim_occur', $nqr->location_claim_occur) == 'Customer' ? 'selected' : '' }}>Customer</option>
                                </select>
                            ...
                            <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <select name="disposition_inventory_location" id="disposition_inventory_location"
                                        ...
                                        class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500">
                                        <option value="">-- Pilih Location --</option>
                                        <option value="At Customer" {{ old('disposition_inventory_location', $nqr->disposition_inventory_location) == 'At Customer' ? 'selected' : '' }}>At
                                            Customer</option>
                                        <option value="At PT.KYBI" {{ old('disposition_inventory_location', $nqr->disposition_inventory_location) == 'At PT.KYBI' ? 'selected' : '' }}>At
                                            PT.KYBI</option>
                                    </select>
                                    <div class="error-message text-red-500 text-xs mt-1 hidden"
                        ...
                        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 mb-6">
                            ...
                            <div>
                                ...
                                <select name="claim_occurence_freq" required
                                    ...
                                    <option value="First Time" {{ old('claim_occurence_freq', $nqr->claim_occurence_freq) == 'First Time' ? 'selected' : '' }}>First Time</option>
                                    <option value="Reoccurred/Routine" {{ old('claim_occurence_freq', $nqr->claim_occurence_freq) == 'Reoccurred/Routine' ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="disposition_defect_part" required
                                    ...
                                    <option value="Keep to Use" {{ old('disposition_defect_part', $nqr->disposition_defect_part) == 'Keep to Use' ? 'selected' : '' }}>Keep to Use
                                    </option>
                                    <option value="Return to Supplier" {{ old('disposition_defect_part', $nqr->disposition_defect_part) == 'Return to Supplier' ? 'selected' : '' }}>Return to
                                        Supplier</option>
                                    <option value="Scrapped at PT.KYBI" {{ old('disposition_defect_part', $nqr->disposition_defect_part) == 'Scrapped at PT.KYBI' ? 'selected' : '' }}>Scrapped
                                        at PT.KYBI</option>
                                </select>
                                <div class="error-message text-red-500 text-xs mt-1 hidden"
                        ...
                        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 mb-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="invoice" value="{{ old('invoice', $nqr->invoice) }}" required
                                    class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
                                    placeholder="INVOICE">
                            ...
                            <div>
                                ...
                                <input type="text" name="total_del" value="{{ old('total_del', $nqr->total_del) }}" required
                                    class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="total_claim" value="{{ old('total_claim', $nqr->total_claim) }}"
                                required
                                class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
                            ...
                            @if($nqr->gambar)
                                <div class="mb-2" id="current-image-container">
                                    <div class="relative inline-block">
                                        <img src="{{ asset('storage/' . $nqr->gambar) }}" alt="Current Image"
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="detail_gambar" name="detail_gambar" rows="3" maxlength="265"
                                class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
                                placeholder="Keterangan detail gambar (opsional)">{{ old('detail_gambar', $nqr->detail_gambar) }}</textarea>
                            <div class="text-xs text-gray-500 mt-1 text-right">
                                <span
                                    id="detail_gambar_count">{{ strlen(old('detail_gambar', $nqr->detail_gambar)) }}</span>/265
                                karakter
                            </div>
                            <script>
                    ...
                    <div class="flex gap-3 justify-end">
                        <button type="submit"
                            ...
                            <img src="{{ asset('icon/edit.ico') }}" alt="save" class="w-4 h-4"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (locationSelect.value) {
                ...
                const oldAction = "{{ old('disposition_inventory_action', $nqr->disposition_inventory_action) }}";
                if (oldAction) {
                    setTimeout(() => {
