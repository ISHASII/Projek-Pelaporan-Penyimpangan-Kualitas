        ...
        <div class="m-0">
            <div
                ...
                <div class="p-6">
                    ...
                    <form id="filter-form" method="GET" action="{{ route('agm.cmr.index') }}" class="mb-4">
                        @php
                            $dateValue = '';
                            if (request('date')) {
                                try {
                                            ...
                                            <td class="px-3 py-3 text-center text-sm hidden sm:table-cell">
                                                <div class="flex flex-col items-center justify-center gap-2">
                                                    <div class="flex items-center justify-center gap-4 action-buttons-container">
                                                        ...
                                                        {{-- Approve/Reject for AGM (visible only to AGM users when Sect & Dept
                                                        approved and AGM pending) --}}
                                                        @if(!is_null($cmr->requested_at_qc) && strtolower($cmr->secthead_status ?? '') === 'approved' && strtolower($cmr->depthead_status ?? '') === 'approved' && $agmStatus === 'pending' && $isAgm)
                                                            <div class="flex flex-col items-center gap-1">
                                                                <button type="button"
                                                                    class="open-approve-modal inline-flex items-center justify-center w-8 h-8 rounded-md hover:bg-green-50"
                                                                    title="Approve" data-url="{{ route('agm.cmr.approve', $cmr->id) }}"
                                                            ...
                                                            <div class="flex flex-col items-center gap-1">
                                                                <button type="button"
                                                                    ...
                                                                    title="Reject" data-url="{{ route('agm.cmr.reject', $cmr->id) }}"
                                                                    data-noreg="{{ $cmr->no_reg }}">
                                                        ...
                                                        {{-- PDF preview (visible if requested) --}}
                                                        @if(!is_null($cmr->requested_at_qc))
                                                            <div class="flex flex-col items-center gap-1">
                                                                <a href="{{ route('agm.cmr.previewFpdf', $cmr->id) }}" target="_blank"
        ...
        <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
            ...
            <div class="mb-4">
                ...
                <div class="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto border rounded p-2 bg-white">
                    @forelse($ppcApprovers ?? [] as $approver)
