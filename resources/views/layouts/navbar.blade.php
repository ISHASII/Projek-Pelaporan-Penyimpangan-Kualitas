...
<head>
    ...
    {{-- Offline Assets: Tailwind CSS, Chart.js, Flatpickr --}}
...
<body class="font-sans bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen antialiased">
            ...
            <div class="flex items-center justify-between h-16 lg:h-18 max-w-screen-xl mx-auto">
                ...
                <div class="flex items-center gap-0 flex-1">
                    <div class="flex-shrink-0 flex items-center py-1 mr-2">
                        <img src="{{ asset('image/KYB.png') }}" alt="Kayaba" draggable="false"
                ...
                <div class="hidden lg:flex items-center space-x-4 flex-shrink-0">
                    ...
                    <div class="relative">
                        <button id="notif-button" aria-haspopup="true" aria-expanded="false"
                            ...
                            {{-- Notification icon (use asset) --}}
                            <span class="relative inline-block">
                                <img src="{{ asset('icon/notif.ico') }}" alt="Notifikasi" class="w-5 h-5 icon-white" />
                                @if($unreadCount > 0)
                                    {{-- small yellow dot on top-right of icon to indicate unread notifications for all
                                    roles --}}
                                    <span
                            ...
                            @if($unreadCount > 0)
                                <span class="ml-2 sr-only">Anda memiliki {{ $unreadCount }} notifikasi belum dibaca</span>
                            @endif
                        </button>
                        ...
                        <div id="notif-dropdown" role="menu" aria-hidden="true"
                            ...
                            <div class="max-h-64 overflow-y-auto">
                                @forelse($notifications->take(6) as $note)
                                    @php
                                    ...
                                    @endphp
                                    <a href="{{ $noteUrl }}" data-notif-id="{{ $note->id }}"
                    ...
                    <div class="text-right border-r border-white/20 pr-4 self-center">
                        <div class="text-sm font-semibold tracking-wide">{{ $user->name ?? 'Guest' }}</div>
                        <div class="text-xs opacity-80 font-medium">{{ ucfirst($user->role ?? 'unknown') }}</div>
                    </div>

                    <form method="POST" action="{{ route('logout') }}" class="logout-form self-center mr-4">
                        @csrf
                        <button type="submit"
                            ...
                            <img src="{{ asset('icon/logout.ico') }}" alt="Logout" class="w-4 h-4 mr-2 icon-white" />
                            Logout
                        </button>
                    </form>
                    ...
                    <div class="text-xs opacity-70 font-medium self-center text-right">
                        {{ $tanggalBaris1 }}
                        <div id="desktop-time">{{ $jamBaris }}</div>
                    </div>
                </div>
                ...
                <div class="lg:hidden flex items-center gap-2">
                    ...
                    <button id="mobile-menu-button"
                        ...
                        <img id="menu-icon" src="{{ asset('icon/hamburger.ico') }}" alt="menu"
                            class="w-6 h-6 nav-transition icon-white" />
                        <img id="close-icon" src="{{ asset('icon/close.ico') }}" alt="close"
                            class="w-6 h-6 hidden nav-transition icon-white" />
                    </button>

                    {{-- Mobile notification dropdown (moved out of the mobile menu so it's always reachable) --}}
                    <div id="mobile-notif-list"
                        ...
                        @forelse(($unreadMobile ?: collect())->take(6) as $note)
                            @php
                                $noteUrl = $note->data['url'] ?? '#';
                                $noReg = $note->data['lpk_no_reg'] ?? null;
                                $act = strtolower($note->data['action'] ?? '');
                            ...
                            <a href="{{ $noteUrl }}" data-notif-id="{{ $note->id }}"
                                ...
                                <div class="flex-1">
                                    ...
                                    @if($noteMessage)
                                        <div class="text-xs text-gray-500">{{ Str::limit($noteMessage, 120) }}</div>
                                    @endif
                                </div>
                            </a>
                        @empty
        ...
        <div id="mobile-menu"
            ...
            <div class="max-w-7xl mx-auto mobile-optimized py-4 space-y-1 custom-scrollbar max-h-96 overflow-y-auto">
                ...
                <div class="pt-4 border-t border-white/20 mt-4">
                    <div class="px-4 py-3 rounded-xl bg-white/10 backdrop-blur-sm">
                        <div class="flex items-center space-x-3">
                            <div class="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center">
                                <img src="{{ asset('icon/akun.ico') }}" alt="User" class="w-6 h-6 icon-white" />
                            </div>
                            <div class="flex-1">
                    ...
                    <form method="POST" action="{{ route('logout') }}" class="mt-3 logout-form">
                        @csrf
                        <button type="submit"
    ...
    {{-- Per-page pushed scripts (approval modal, notif toggles) --}}
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
    ...
    {{-- Render pushed scripts from child views (e.g. approval modal JS) --}}
    @stack('scripts')
