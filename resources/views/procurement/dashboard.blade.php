...
@section('content')
                        ...
                        <div class="flex items-start justify-between">
                            <div>
                                ...
                                <p class="text-lg sm:text-xl md:text-2xl font-bold text-green-600">
                                    {{ number_format($cmrStats['total'] ?? 0) }}
                                </p>
                                <div class="flex items-center mt-1 text-xs sm:text-sm">
                                    <span class="text-green-600 font-medium">{{ $cmrStats['completed'] ?? 0 }}</span>
                                    <span class="text-gray-500 ml-1">completed</span>
                                </div>
                            </div>
                            <div
                    ...
                    <div
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            <div class="p-2 rounded-lg bg-green-50 text-center">
                                ...
                                <div class="text-sm font-bold text-green-600">{{ $cmrStats['completed'] ?? 0 }}</div>
                            </div>
                            <div class="p-2 rounded-lg bg-yellow-50 text-center">
                                <div class="text-xs text-gray-600">Pending</div>
                                <div class="text-sm font-bold text-yellow-600">{{ $cmrStats['pending'] ?? 0 }}</div>
                            </div>
                            <div class="p-2 rounded-lg bg-red-50 text-center">
                                <div class="text-xs text-gray-600">Rejected</div>
                                <div class="text-sm font-bold text-red-600">{{ $cmrStats['rejected'] ?? 0 }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <div class="grid grid-cols-3 gap-2 mt-2 sm:mt-4">
                            <div class="p-3 rounded-lg bg-green-50 text-center">
                                <div class="text-xs text-gray-600">Completed</div>
                                <div class="text-sm font-bold text-green-600">{{ $nqrStats['completed'] ?? 0 }}</div>
                            </div>
                            <div class="p-3 rounded-lg bg-yellow-50 text-center">
                                <div class="text-xs text-gray-600">Pending</div>
                                <div class="text-sm font-bold text-yellow-600">{{ $nqrStats['pending'] ?? 0 }}</div>
                            </div>
                            <div class="p-3 rounded-lg bg-red-50 text-center">
                                <div class="text-xs text-gray-600">Rejected</div>
                                <div class="text-sm font-bold text-red-600">{{ $nqrStats['rejected'] ?? 0 }}</div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const nqrTrendData = {!! json_encode($monthlyData['nqr'] ?? []) !!}.slice();
            const cmrTrendData = {!! json_encode($monthlyData['cmr'] ?? []) !!}.slice();
            ...
            let monthlyChart = new Chart(monthlyCtx, {
                ...
                data: {
                    ...
                    datasets: [
                        {
                            label: 'NQR',
                            data: {!! json_encode($monthlyData['nqr'] ?? []) !!},
                        ...
                        {
                            label: 'CMR',
                            data: {!! json_encode($monthlyData['cmr'] ?? []) !!},
