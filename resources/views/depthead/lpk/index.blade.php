    ...
    <div class="m-0">
        <div class="bg-white rounded-none shadow-none overflow-hidden border-t-0 border-l-0 border-r-0 border-gray-100 w-full">
            <div class="p-6">
                ...
                <form id="filter-form" method="GET" action="{{ route('depthead.lpk.index') }}" class="mb-4">
                    @php
                    ...
                    <div class="rounded-md border border-gray-200 p-3 sm:p-4 bg-white shadow-sm">
                        ...
                        <div class="block lg:hidden space-y-2">
                            <div>
                                ...
                                <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari no reg, supplier, part..." class="mt-1 block w-full rounded-md border border-gray-300 bg-white text-sm px-2.5 py-1.5 focus:ring-1 focus:ring-yellow-500 focus:border-yellow-500" />
                            </div>

                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    <label class="text-xs text-gray-600 font-medium">Tanggal</label>
                                    <input type="text" id="date-picker-lpk-mobile" name="date_display" value="{{ $dateValue }}" placeholder="dd-mm-yyyy" readonly class="mt-1 block w-full rounded-md border border-gray-300 bg-white text-sm px-2.5 py-1.5 focus:ring-1 focus:ring-yellow-500 focus:border-yellow-500" />
                                </div>
                                <div>
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <select name="status_lpk" class="mt-1 block w-full rounded-md border border-gray-300 bg-white text-sm px-2.5 py-1.5 focus:ring-1 focus:ring-yellow-500 focus:border-yellow-500">
                                        <option value="">Semua</option>
                                        <option value="claim" {{ request('status_lpk') == 'claim' ? 'selected' : '' }}>Claim</option>
                            ...
                            <div class="grid grid-cols-2 gap-2 pt-1">
                                ...
                                <a href="{{ route('depthead.lpk.index') }}" class="inline-flex justify-center items-center px-3 py-1.5 bg-white hover:bg-gray-50 border border-gray-300 text-gray-700 text-sm font-medium rounded-md transition-colors">Reset</a>
                            </div>
                        </div>
                        ...
                        <div class="hidden lg:flex gap-2 items-end">
                            <div class="flex-1 min-w-0">
                                <label class="text-xs text-gray-600 font-medium">Pencarian</label>
                                <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari no reg, supplier, part, PO, deskripsi..." class="mt-1 block w-full rounded-md border border-gray-300 bg-white text-sm px-2.5 py-1.5 focus:ring-1 focus:ring-yellow-500 focus:border-yellow-500" />
                            </div>

                            <div class= "w-36">
                                <label class="text-xs text-gray-600 font-medium">Tanggal</label>
                                <input type="text" id="date-picker-lpk" name="date_display" value="{{ $dateValue }}" placeholder="dd-mm-yyyy" readonly class="mt-1 block w-full rounded-md border border-gray-300 bg-white text-sm px-2.5 py-1.5 focus:ring-1 focus:ring-yellow-500 focus:border-yellow-500" />
                            </div>
                            ...
                            <div class="flex gap-2 items-center flex-shrink-0">
                                ...
                                <a href="{{ route('depthead.lpk.index') }}" class="inline-flex items-center px-3 py-1.5 bg-white hover:bg-gray-50 border border-gray-300 text-gray-700 text-sm font-medium rounded-md whitespace-nowrap transition-colors">Reset</a>
                            </div>
                        </div>
                    </div>
                </form>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 table-fixed">
                            ...
                            <tbody class="bg-white divide-y divide-gray-100">
                                @foreach($lpks as $i => $lpk)
                                    <tr class="odd:bg-gray-100 even:bg-white hover:bg-gray-200 transition-colors" data-lpk-id="{{ $lpk->id }}">
                                        <td class="px-3 py-3 text-sm text-gray-900">{{ $lpk->no_reg }}</td>
                                        <td class="px-3 py-3 text-sm text-gray-900">{{ $lpk->tgl_terbit ? (is_string($lpk->tgl_terbit) ? (strtotime($lpk->tgl_terbit) ? date('d-m-Y', strtotime($lpk->tgl_terbit)) : '') : $lpk->tgl_terbit->format('d-m-Y')) : '' }}</td>
                                        ...
                                        <td class="px-3 py-3 text-sm text-gray-900 align-middle">
                                            @if(!empty($lpk->problem))
                                                ...
                                                <div class="truncate" style="max-width:40ch;" title="{{ $lpk->problem }}">{{ $short }}</div>
                                            @else
                                                &mdash;
                                            @endif
                                        </td>
                                        <td class="px-3 py-3 text-sm text-gray-900 text-center">
                                            ...
                                            @endphp
                                            @if($statusText)
                                                <span class="inline-flex items-center justify-center px-2 py-1 text-xs font-medium rounded-full {{ $badgeClass }}">
                                                    {{ $statusText }}
                                                </span>
                                            @else
                                        ...
                                        <td class="px-3 py-3 text-sm text-gray-900 status-approval-cell">
                                            ...
                                            @endphp
                                            <div class="font-medium status-text">{{ $statusMsg }}</div>
                                        </td>
                                            ...
                                            <div class="flex flex-col items-center justify-center gap-2">
                                                <div class="flex items-center justify-center gap-4 action-buttons-container">
                                                    @php
                                                        $dept = strtolower($lpk->depthead_status ?? 'pending');
                                                    @endphp
                                                    @if(!is_null($lpk->requested_at_qc) && $dept === 'pending')
                                                        @php $sect = strtolower($lpk->secthead_status ?? 'pending'); @endphp
                                                        @if($sect === 'approved')
                                                            <div class="flex flex-col items-center gap-1">
                                                                <button type="button"
                                                                    class="open-approve-modal inline-flex items-center justify-center w-8 h-8 rounded-md hover:bg-green-50 transition"
                                                                    title="Setuju"
                                                                    data-url="{{ route('depthead.lpk.approve', $lpk->id) }}"
                                                                    data-noreg="{{ $lpk->no_reg }}"
                                                                    data-lpk-id="{{ $lpk->id }}">
                                                            ...
                                                            <div class="flex flex-col items-center gap-1">
                                                                <button type="button"
                                                                    ...
                                                                    title="Tolak"
                                                                    data-url="{{ route('depthead.lpk.reject', $lpk->id) }}"
                                                                    data-noreg="{{ $lpk->no_reg }}"
                                                                    data-lpk-id="{{ $lpk->id }}">
                                                    ...
                                                    @if(!is_null($lpk->requested_at_qc))
                                                        <div class="flex flex-col items-center gap-1">
                                                            <a href="{{ route('depthead.lpk.previewPdf', $lpk->id) }}" target="_blank" class="inline-flex items-center justify-center w-8 h-8 rounded-md hover:bg-gray-100 transition" title="Preview PDF">
                ...
                {{-- Pagination inside card area for consistent spacing --}}
                @if(method_exists($lpks, 'links'))
                    <div class="mt-6 border-t border-gray-100 pt-4">
                        <div class="flex flex-col sm:flex-row items-center justify-between gap-3">
                            <div class="text-sm text-gray-600">
                                Menampilkan <span class="font-medium">{{ $lpks->firstItem() ?? 0 }}</span> - <span class="font-medium">{{ $lpks->lastItem() ?? 0 }}</span> dari <span class="font-medium">{{ $lpks->total() }}</span> data
                            </div>

                            <nav class="flex items-center gap-3" aria-label="Pagination">
                                @php $prev = $lpks->previousPageUrl(); $next = $lpks->nextPageUrl(); @endphp

                                <a href="{{ $prev ?: '#' }}" class="inline-flex items-center gap-2 px-3 py-2 rounded-lg border {{ $lpks->onFirstPage() ? 'text-gray-400 border-gray-200 pointer-events-none bg-white' : 'text-gray-600 border-gray-200 bg-white hover:bg-gray-50 shadow-sm' }}" aria-disabled="{{ $lpks->onFirstPage() ? 'true' : 'false' }}">
                                ...
                                <div class="hidden sm:inline-flex items-center px-3 py-2 bg-white border border-gray-100 rounded-full shadow-sm text-sm text-gray-700">
                                    Halaman <span class="mx-2 font-semibold">{{ $lpks->currentPage() }}</span> dari <span class="mx-2 font-medium">{{ $lpks->lastPage() }}</span>
                                </div>

                                <a href="{{ $next ?: '#' }}" class="inline-flex items-center gap-2 px-3 py-2 rounded-lg border {{ $lpks->hasMorePages() ? 'text-gray-600 border-gray-200 bg-white hover:bg-gray-50 shadow-sm' : 'text-gray-400 border-gray-200 pointer-events-none bg-white' }}" aria-disabled="{{ $lpks->hasMorePages() ? 'false' : 'true' }}">
                                    <span class="text-sm">Berikutnya ></span>
                                </a>

                                <div class="sm:hidden px-3 py-1 text-xs text-gray-600">Hal. <span class="font-medium">{{ $lpks->currentPage() }}</span>/<span class="font-medium">{{ $lpks->lastPage() }}</span></div>
                            </nav>
                        </div>
                    </div>
                @endif
