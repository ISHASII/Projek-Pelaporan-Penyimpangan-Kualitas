    ...
    <div class="m-0">
        <div class="bg-white rounded-none shadow-none overflow-hidden border-t-0 border-l-0 border-r-0 border-gray-100 w-full">
            <div class="p-6">
                ...
                <form id="filter-form" method="GET" action="{{ route('depthead.cmr.index') }}" class="mb-4">
                    @php
                        $dateValue = '';
                        if (request('date')) {
                            try {
                    ...
                    <div class="rounded-md border border-gray-200 p-3 sm:p-4 bg-white shadow-sm">
                        ...
                        <div class="block lg:hidden space-y-2">
                            <div>
                                ...
                                <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari no reg, supplier, part..." class="mt-1 block w-full rounded-md border border-gray-300 bg-white text-sm px-2.5 py-1.5" />
                            </div>

                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    <label class="text-xs text-gray-600 font-medium">Tanggal</label>
                                    <input type="text" id="date-picker-cmr-mobile" name="date_display" value="{{ $dateValue }}" placeholder="dd-mm-yyyy" readonly class="mt-1 block w-full rounded-md border border-gray-300 bg-white text-sm px-2.5 py-1.5" />
                                </div>
                                <div>
                            ...
                            <div class="grid grid-cols-2 gap-2 pt-1">
                                ...
                                <a href="{{ route('depthead.cmr.index') }}" class="inline-flex justify-center items-center px-3 py-1.5 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-md">Reset</a>
                            </div>
                        </div>
                        ...
                        <div class="hidden lg:flex gap-2 items-end">
                            <div class="flex-1 min-w-0">
                                ...
                                <input type="text" name="q" value="{{ request('q') }}" placeholder="Search no reg, supplier, part, PO..." class="mt-1 block w-full rounded-md border border-gray-300 bg-white text-sm px-2.5 py-1.5" />
                            </div>

                            <div class="w-36">
                                <label class="text-xs text-gray-600 font-medium">Date</label>
                                <input type="text" id="date-picker-cmr" name="date_display" value="{{ $dateValue }}" placeholder="dd-mm-yyyy" readonly class="mt-1 block w-full rounded-md border border-gray-300 bg-white text-sm px-2.5 py-1.5" />
                            </div>
                            ...
                            <div class="flex gap-2 items-center flex-shrink-0">
                                ...
                                <a href="{{ route('depthead.cmr.index') }}" class="inline-flex items-center px-3 py-1.5 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-md">Reset</a>
                            </div>
                        </div>
                    </div>
                </form>
                                ...
                                @foreach($cmrs as $i => $cmr)
                                    <tr class="odd:bg-gray-100 even:bg-white hover:bg-gray-200 transition-colors" data-cmr-id="{{ $cmr->id }}">
                                        ...
                                        <td class="px-3 py-3 text-sm text-gray-900 text-center">
                                            ...
                                            @endphp
                                            <span class="inline-flex items-center justify-center px-2 py-1 text-xs font-medium rounded-full {{ $badgeClass }}">{{ $prodText }}</span>
                                        </td>
                                        ...
                                        <td class="px-3 py-3 text-center text-sm hidden sm:table-cell action-buttons-container">
                                            <div class="flex flex-col items-center justify-center gap-2">
                                                <div class="flex items-center justify-center gap-4">
                                                    @php $deptStatus = strtolower($cmr->depthead_status ?? 'pending'); @endphp
                                                    @if(!is_null($cmr->requested_at_qc) && strtolower($cmr->secthead_status ?? '') === 'approved' && $deptStatus === 'pending')
                                                        <div class="flex flex-col items-center gap-1">
                                                            <button type="button"
                                                                class="open-approve-modal inline-flex items-center justify-center w-8 h-8 rounded-md hover:bg-green-50"
                                                                title="Setuju"
                                                                data-url="{{ route('depthead.cmr.approve', $cmr->id) }}"
                                                                data-cmr-id="{{ $cmr->id }}"
                                                        ...
                                                        <div class="flex flex-col items-center gap-1">
                                                            <button type="button"
                                                                ...
                                                                title="Tolak"
                                                                data-url="{{ route('depthead.cmr.reject', $cmr->id) }}"
                                                                data-cmr-id="{{ $cmr->id }}"
                                                                data-noreg="{{ $cmr->no_reg }}">
                                                    ...
                                                    @if(!is_null($cmr->requested_at_qc))
                                                        <div class="flex flex-col items-center gap-1">
                                                            <a href="{{ route('depthead.cmr.previewFpdf', $cmr->id) }}" target="_blank" class="inline-flex items-center justify-center w-8 h-8 rounded-md hover:bg-gray-100" title="Preview PDF">
                    ...
                    <div class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
                        <div class="flex items-center justify-between">
                            <nav class="flex items-center justify-center space-x-2 sm:justify-between w-full">
                                @php $prev = $cmrs->previousPageUrl(); $next = $cmrs->nextPageUrl(); @endphp
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateRowAfterAction(cmrId, newStatus, hideActions) {
        ...
        if (hideActions) {
            ...
            if (actionCell) {
                actionCell.innerHTML = '<div class="flex flex-col items-center gap-1"><a href="{{ url("depthead/cmr") }}/' + cmrId + '/preview-fpdf" target="_blank" class="inline-flex items-center justify-center w-8 h-8 rounded-md hover:bg-gray-100" title="Preview PDF"><img src="{{ asset("icon/pdf.ico") }}" alt="PDF" class="w-4 h-4" /></a><span class="text-xs text-gray-500 mt-1">PDF</span></div>';
            }
        }
    }
