		...
		<div class="m-0">
			<div
				...
				<div class="p-6">
					...
					<form id="filter-form" method="GET" action="{{ route('ppchead.cmr.index') }}" class="mb-4">
													...
													<div class="flex items-center justify-center gap-4 action-buttons-container">
														...
														<div class="flex flex-col items-center justify-center gap-2">
															...
															<div class="flex flex-row items-center justify-center gap-6">
																@if(!is_null($cmr->requested_at_qc) && $sect === 'approved' && $dept === 'approved' && $agm === 'approved' && $ppc === 'pending' && $statusApproval === 'waiting for ppc head approval')
																	<div class="flex flex-col items-center gap-1">
																		<a href="{{ route('ppchead.cmr.ppcForm', $cmr->id) }}"
                                                                ...
                                                                @if(!is_null($cmr->requested_at_qc) && $sect === 'approved' && $dept === 'approved' && $agm === 'approved' && $ppc === 'pending' && $statusApproval === 'waiting for ppc head approval')
																	<div class="flex flex-col items-center gap-1">
																		<button type="button"
																			...
																			title="Tolak"
																			data-url="{{ route('ppchead.cmr.reject', $cmr->id) }}"
																...
																@if(!is_null($cmr->requested_at_qc))
																	<div class="flex flex-col items-center gap-1">
																		<a href="{{ route('ppchead.cmr.previewFpdf', $cmr->id) }}"
																			target="_blank"
							...
							<div class="flex items-center justify-between">
								<nav class="flex items-center justify-center space-x-2 sm:justify-between w-full">
									...
									<div
										...
										Page <span class="mx-2 font-semibold">{{ $cmrs->currentPage() }}</span> of <span
