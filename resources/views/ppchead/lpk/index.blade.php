        ...
        <div class="m-0">
            <div
                ...
                <div class="p-6">
                    ...
                    <form id="filter-form" method="GET" action="{{ route('ppchead.lpk.index') }}" class="mb-4">
                        @php
                            ...
                            <div class="hidden lg:flex gap-2 items-end">
                                ...
                                <div class="w-36">
                                    ...
                                    <select name="status_lpk"
                                        ...
                                        <option value="complaint" {{ request('status_lpk') == 'complaint' ? 'selected' : '' }}>Complaint (Informasi)</option>
                                    </select>
                                </div>
                                ...
                                <tbody class="bg-gray-50 divide-y divide-gray-200">
                                    @foreach($lpks as $lpk)
                                        <tr class="{{ $loop->even ? 'bg-gray-100' : 'bg-white' }} hover:bg-gray-200 transition-colors"
                                            data-lpk-id="{{ $lpk->id }}">
                                            <td class="px-4 py-4 text-sm text-gray-900">{{ $lpk->no_reg ?? $lpk->id }}</td>
                                            <td class="px-4 py-4 text-sm text-gray-600">
                                                {{ $lpk->tgl_terbit ? (is_string($lpk->tgl_terbit) ? (strtotime($lpk->tgl_terbit) ? date('d-m-Y', strtotime($lpk->tgl_terbit)) : $lpk->tgl_terbit) : $lpk->tgl_terbit->format('d-m-Y')) : (optional($lpk->created_at)->format('d-m-Y') ?? '-') }}
                                            </td>
                                            <td class="px-4 py-4 text-sm text-gray-600">{{ $lpk->nama_supply ?? '-' }}</td>
                                            <td class="px-4 py-4 text-sm text-gray-600">{{ $lpk->nama_part ?? '-' }}</td>
                                            <td class="px-4 py-4 text-sm text-gray-600">{{ $lpk->nomor_part ?? '-' }}</td>
                                                    ...
                                                    <div class="flex items-center justify-center gap-4">
                                                        ...
                                                        <div class="flex flex-col items-center justify-center gap-2">
                                                            ...
                                                            <div
                                                                ...
                                                                @if(!is_null($lpk->requested_at_qc) && $sect === 'approved' && $dept === 'approved' && $ppc === 'pending')
                                                                    <div class="flex flex-col items-center gap-1 approve-btn-wrapper">
                                                                        <a href="{{ route('ppchead.lpk.ppcForm', $lpk->id) }}"
                                                                ...
                                                                @if(!is_null($lpk->requested_at_qc) && $sect === 'approved' && $dept === 'approved' && $ppc === 'pending')
                                                                    <div class="flex flex-col items-center gap-1 reject-btn-wrapper">
                                                                        <button type="button"
                                                                            ...
                                                                            title="Tolak"
                                                                            data-url="{{ route('ppchead.lpk.reject', $lpk->id) }}"
                            ...
                            <div class="flex flex-col sm:flex-row items-center justify-between gap-3">
                                <div class="text-sm text-gray-600">
                                    Menampilkan <span class="font-medium">{{ $lpks->firstItem() ?? 0 }}</span> - <span
                                        ...
                                        class="font-medium">{{ $lpks->total() }}</span>
                                </div>
